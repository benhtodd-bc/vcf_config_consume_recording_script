...
6. Monitor Recenter Tasks to verify the ```vks-cluster-1``` tasks.
    ...
    The VKS Cluster creation typically takes about 60 to 70 mins for the deployment to complete.
...
6. Verify that ```vks-cluster-1-control-plane-service``` is assigned an ```External IP 10.10.10.xx``` with ports ```6443/TCP```.

7. Click ```vks-cluster-1-control-plane-service``` to review the Service 
Details.
